...
def process_registration (message, text, parse_func):
    try:
        (location, patient_id, patient_conn, contact_time, lang) = parse_func(text, message.connection)
    ...
    schedule_notifications(reg, message.date)
